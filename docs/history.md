...
- ~~Range requests, `sendfile`, and serving a file too big to hold in memory~~ ([ADR 0010](./adr/0010-static-files-are-held-in-memory.md)). *All three landed in 0.1.0.* Ranges arrived as the addition ADR 0010 predicted ([ADR 0021](./adr/0021-a-range-is-a-slice-and-two-headers.md)); the other two waited for the Engine to supply the seam this ADR had described in advance, and then cost four names on the Bulkhead ([ADR 0037](./adr/0037-a-file-too-big-to-hold-is-opened-not-read.md)).
...
- **"Static files are deeper than they look — range requests, caching, `sendfile`."** Serving from memory kept the first version small, caching came along free with it, and range requests arrived later as an addition rather than a rewrite ([ADR 0010](./adr/0010-static-files-are-held-in-memory.md), [ADR 0021](./adr/0021-a-range-is-a-slice-and-two-headers.md)). `sendfile` was the one part left, and it landed the same way for the same reason — the prediction that it would be "the same `find`, a different way of getting at the bytes" is what it turned out to be ([ADR 0037](./adr/0037-a-file-too-big-to-hold-is-opened-not-read.md)). The risk was named as depth; what it actually was, was sequencing.
...
## The refusal that expired, and the hole nobody had written down

The roadmap's third item said `sendfile` "contradicts [ADR 0010](./adr/0010-static-files-are-held-in-memory.md) rather than extending it, so it wants its own argument before any code". The argument turned out to be mostly a matter of checking whether the old one still held. It did not, and nothing in the repository could have said so — the fact that changed was in a dependency.

**The premise had expired quietly.** ADR 0010 chose to hold files in memory over adding file IO to the Bulkhead, and priced option A as "open, stat, read, seek, errors, cancellation — owed by every replacement Engine forever". That was true when written. It stopped being true when `sendFile` became a slot in the `std.Io.Writer` vtable and zio 0.17 — the version already pinned — filled it in, along with `openat` through the event loop and a `std.Io.File.Reader`. The obligation did not shrink; it stopped being zfast's to define. What the Bulkhead actually grew was four names.

**The seam had been named three stages earlier.** ADR 0010's last paragraph: *"If a future Engine ever does want to supply file IO, `std.Io` is already the shape the seam would take."* It is the shape it took. A decision that writes down the conditions under which it should be revisited is a decision that can be revisited by reading rather than by arguing, and this one cost an afternoon of reading a dependency instead of a design.

**The bigger gap was not on the roadmap at all.** The item tracked was "the static tree cannot serve a large file". What nobody had written down is that a handler could not serve a file *at all* — no `http.ServeFile`, no `res.sendFile`, nothing. An invoice behind an authorisation check is an ordinary endpoint for the applications zfast says it is for, and it was unwritable. The roadmap tracked the half with an ADR attached to it, because that is the half that had generated an argument. Absence generates no argument.

**The obvious ETag was the wrong one.** A file too big to hash at load wants a validator built from something cheap, and a *weak* one looks like the honest answer — it says "equivalent", not "identical", which is exactly what mtime and size can promise. RFC 9110 then makes it useless: an `If-Range` carrying a weak validator must be ignored, so every client resuming a large download would start again from zero. Resuming is what large files are for. The tag is strong, built from the modification time and the size, which is what nginx has served by default for twenty years — and the risk being taken is stated rather than discovered.

**One knob changed meaning instead of two knobs existing.** `max_file_bytes` was the ceiling a file was refused at; it is now the line a file stops being held at. Below it nothing changed at all — same bytes, same hash, same gzip, same benchmark. A second option would have needed a paragraph explaining how it related to the first one, and that paragraph is the tax that gets paid on every reading of the documentation forever.

**Two bugs came out of reading rather than failing.** `std.Io.Limit` is `usize`-backed, so on a 32-bit build a file over four gigabytes clamps — and a complete transfer would have been read as a short send and had its connection closed for it. And `load` had a latent double free on the path where the SPA fallback is missing, where `set.deinit()` ran while an `errdefer` on the same slice was still live. Neither had a failing test to find it, because neither is reachable on the machine the tests run on.

**And one guard was proved by breaking it.** A file response holds a descriptor for as long as the send takes, closed on all eight exits including the error ones. The test counts `/proc/self/fd` across a request — and it was checked by deleting the `defer` and watching it fail with "expected 26, found 46", which is [ADR 0033](./adr/0033-a-guard-is-not-a-guard-until-it-has-been-seen-to-fail.md) applied to a property that would otherwise only ever have been observed holding.
