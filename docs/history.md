...
**One pass: 1,581 occurrences across 216 files, and 1,212/1,212 tests on both sides of it.** The import name is the consumer's to choose — their `build.zig` names the module, and `src/names.zig` rewrites every type our own compile errors print — so no path inside the framework depended on the spelling. The only step that is not textual is `build.zig.zon`'s fingerprint, which has the package name hashed into its low half and has to be regenerated from the value the compiler prints in the error.
