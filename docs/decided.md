...
| An aggregate's `.where` and a `nilo_children` entry's `.where` take a value, `null`, the six comparisons, `.in` and `.not_in` over the table's own columns (an aggregate's through a reference too), and nothing else ([ADR 218](./adr/218-a-row-may-carry-its-parent-its-children-or-a-sum.md)) | the values are written into the statement, because the entry is part of the Row and has nothing to bind; a pattern, `.any` or `.exists` goes in the read's own `.where`, or `db.raw` | a filter from a real screen that the read's `.where` cannot carry instead |
