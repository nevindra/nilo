...
7. **A narrower Row may say it has a parent, children, or is a group, and the call site does not change.** A field typed as another Row is joined and read into it; a `[]const` field of one is read by a second statement, matched to its parent by position, in an order and under a condition `nilo_children` may give it; a count of the rows pointing back is an `i64` field read by a correlated subquery; `nilo_aggregate` makes the Row one row per group, with `WHERE` and `HAVING` split by what each condition names, and an entry's own `.where` a `FILTER` written with its values in. [ADR 218](../adr/218-a-row-may-carry-its-parent-its-children-or-a-sum.md)
