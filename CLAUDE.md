...
- **`docs/adr/`** — 191 binding decisions, each naming the alternative it
...
and the six `refusals` steps check the wording of 233 error messages. Prefer making
...
zig build refusals     # the framework's 118 compile-error checks — NOT the others
...
that failed, so all 118 are re-analysed every run. They stay on `test` on
