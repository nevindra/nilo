...
- **`docs/adr/`** — 193 binding decisions, each naming the alternative it
...
and the six `refusals` steps check the wording of 238 error messages. Prefer making
...
zig build refusals     # the framework's 123 compile-error checks — NOT the others
...
that failed, so all 123 are re-analysed every run. They stay on `test` on
