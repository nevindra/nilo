...
- **An aggregate may carry a `.where`**: `.idr = .{ .sum = .amount_minor, .where = .{ .currency = "IDR" } }` is `sum(…) FILTER (WHERE …)` on both databases, with the values written in. A filtered `sum`, `min`, `max` or `avg` is optional whatever its column; count matching rows with `.{ .count = .id, .where = … }` (ADR 218).
