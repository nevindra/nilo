...
- **`rawPage` and `rawPageOrdered` refuse an `OFFSET` nilo cannot set back to 0**: one written as a number other than 0, as arithmetic, or with a placeholder another clause also uses, values handed over as a named struct, and SQLite's `LIMIT a, b`. What to change: write `OFFSET $n` (`$n::int` is fine) and work the number out in Zig. It is what lets a page past its last row say its total (ADR 205).
...
- **`.set = .{ .start_date = .today }` stamps a `sql.Date` column with the database's date**, `CURRENT_DATE`, and `.now` and `.today` stand in a condition too: `.where = .{ .start_date = .{ .gt = .today } }`, nothing bound. Each is refused on the other's column type (ADR 181).
- **`.ieq` and `.not_ieq`: equality that ignores case**, `lower("email") = lower($1)` on Postgres and `COLLATE NOCASE` on both sides on SQLite, the expression a `.unique` with `.ignoring_case` indexes. The lookup the upsert refusal already named, and an `_` in an address is a character rather than `.ilike`'s wildcard (ADR 181).
- **`.in` and `.not_in` take `sql.given`.** `.stage = .{ .in = sql.given(q.stages) }` drops the term when the list is null and keeps it when there is one, empty included, so a multi-select on a filter bar is one statement. It was a Refusal that read an absent list as an empty one (ADR 149).
- **`db.rawPageOrdered` and `tx.rawPageOrdered`**: a raw statement read as a page, with the request's `ORDER BY` through `{order}`. A sorted list screen was `rawOrdered` for the rows and a second statement for the total with the `WHERE` pasted in again (ADR 205).
- **`.order` on a narrower Row may name a column of its table the Row does not carry**, a tiebreak the response has no reason to show. On a grouped Row it is a Refusal (ADR 218).
- **An aggregate may carry a `.where`**: `.idr = .{ .sum = .amount_minor, .where = .{ .currency = "IDR" } }` is `sum(…) FILTER (WHERE …)` on both databases, with the values written in. A filtered `sum`, `min`, `max` or `avg` is optional whatever its column; count matching rows with `.{ .count = .id, .where = … }`. A column with a `.references` is a way into the row it points at, `.where = .{ .state_id = .{ .category = .done } }`, joined once (ADR 218).
- **`pub const nilo_children`: an order and a condition for a children field, and a count of children.** `.lines = .{ .order = .{ .position = .asc }, .where = … }` orders and narrows the second statement; `.line_count = .{ .count = Line }` is an `i64` field read by a correlated `count(*)` in the same statement, sortable and usable in `.where`, with no child read (ADR 218).
- **`sql.Sent.route`**: the `operationId` of the route whose request sent the statement, read off the Scope, null under a `Run`. A watcher can say which page a slow statement belongs to (ADR 108).
- **`db.rawExplain(c, sql, values)` and `db.rawExplainOrdered(c, sql, values, order)`**: `db.explain` for a statement nilo did not write, inside a transaction that is rolled back, since `ANALYZE` runs what it plans (ADR 232).
- **`.where` on a narrower Row may name a column of its table the Row does not carry**, as `.order` already could, bound as the table's column type (ADR 218).
- **`.today` on `sql.AsText("date")` and `.now` on `sql.AsText("timestamptz")`**, in a `.set` and in a condition: the database writes the value either way. On SQLite `.now` there is RFC 3339 text rather than microseconds (ADR 181).
- **`db.explain(Row, c, options)`**: the plan of the statement `db.select` would send with the same values, `EXPLAIN (ANALYZE, BUFFERS)` on Postgres and `EXPLAIN QUERY PLAN` on SQLite, for a test to hold a plan to what it should be (ADR 232).
- **`routeNameOf(scope)`** in `nilo_core`, beside `requestIdOf`: a Scope's route name, or null for one without a `routeName` (ADR 108).
...
- **A page past its last row answers its real total**, where it answered 0: `db.page` and `tx.page` send one `db.count` with the page's `.where` when a page that skipped rows comes back empty, and `rawPage` and `rawPageOrdered` ask the same statement again from its first row (ADR 150, ADR 205).
- **An order term that says where NULLs go compiles on a shaped Row**, through a parent and on an aggregate; it failed inside `shape.zig` (ADR 218).
...
- **On SQLite, an `INSERT … RETURNING` that breaks a constraint reports what SQLite said**, to the watcher and to `sql.problem`. SQLite reports it at the first step, and only the send was asked. `Problem.constraint` is filled on SQLite too, from the text after `constraint failed:` (ADR 117).
